...
Module ID / name / version: `streamingunity-hermes-v1` / **StreamingUnity Hermes** / `1.0.0-beta.4`
...
ZIP SHA-256: `1311673eec5f06ad86e12ea3db2d2846ba548925b99f38240309a13ac119ec2c`
...
Test date/time: 2026-09-18, ~20:50–24:00 local (UTC+2). Network: residential, Cloudflare-fronted hosts.
...
Also exercised: 1966 catalogue title in search, 2026 releases, multi-season shows, non-Latin titles, empty search, invalid ids, TV/Movies grid paging.
...
- **Site assertions: 39/39 passed** (`module-tests/site_checks.cjs`):
  ...
  - discovery feeds: trending/latest page 1+2 (50 items/page, **0 overlap**); **TV/Movies grid feeds now page deep — p1/p2 50 items each, 0 overlap, strict type filter (`tv`/`movie`), stops cleanly at the source's guest cap (page 18 → empty, `hasMore:false`)**; top10; unknown feed degrades to empty
...
- **Fix shipped in beta.2:** community test identity `SP-VID-9004-STREAMINGUNITY-HERMES-TEST` / number `9004`
  ...
  (SP-VID-065).
- **Repository state (as of beta.4):** bundle `Testing-6` advertises beta.4; beta.1–beta.3 ZIPs remain on
...
| Repository index + bundle rebuild | 1 | 1 | 0 | `repository.json` rebuilt (bundle 4 at repair time; 5 for beta.3, 6 for beta.4); module/bundle SHA-256 recomputed and checked |
...
| --- | --- | --- | --- |
| Site-side assertions (`site_checks.cjs`) | 39 | 39 | 0 |
...
Defects found during testing and fixed before packaging: (1) TV/Movies feeds stopped after 30 items (`hasMore:false`) → beta.4 pages via the source's infinite-scroll JSON endpoint with strict type filter; (2) cross-section dedupe used to shrink the Top-10 row → beta.3 reserves the ranked ten before emitting rows (hero first); (3) audio-less movie master was mislabeled “Italian” → now `Auto`/`und`; (4) default-track ordering regression → default first; (5) bare-id URLs 404 → placeholder slug; (6) repository install rejected development identity `0` → beta.2 ships community test identity `9004` and a collision-free module id. One test-harness false alarm (recon scripts hardcoded `h=1`) is documented; it was never a module defect.

## Verified / flaky / unverified

- **Verified (local, this machine):** catalogue, search, details, season/episode lists, discovery rows+feeds, **home row order + counts after reservation**, **TV/Movies feed deep paging (pages 1–2 disjoint, strict type filter, guest cap), pagination without duplicates**, stream resolution for 6 samples, crypto/segment structure, subtitle files + cue timing, ZIP integrity (bytes, hash, packaged smoke test), repository index/bundle rebuild + hashes, **in-app repository install of the repaired identity (operator side)**.
...
- **Unverified:** in-app **playback** (picture+sound), seek/pause, audio-track switching, subtitle rendering, downloads/offline, physical devices, **S2 native playback**, FHD (1080p) while guests are gated, movie audio language (muxed, not identifiable locally), in-app scroll of the new TV/Movies paging (operator to re-test).

## Final verdict

- Local checks: **PASS** (with the two documented kit-guard blockers)
- Repository install (identity): **fixed**; in-app install confirmed by the operator; row order fixed in beta.3; TV/Movies grid scrolling fixed in beta.4
...
**Version:** 1.0.0-beta.4 · **ZIP:** `Testing-Modules-1234/modules/StreamingUnityHermes-1.0.0-beta.4.zip` · **SHA-256:** `1311673eec5f06ad86e12ea3db2d2846ba548925b99f38240309a13ac119ec2c`
**Counts:** 82 attempted / 80 passed / 2 blocked-by-kit-guard (see table) · failure reasons: none from the module; the 2 blocked probes are the kit's deliberate AES-128 refusal
