...
Module ID / name / version: `streamingunity-hermes-v1` / **StreamingUnity Hermes** / `1.0.0-beta.5`
...
ZIP SHA-256: `a084a623e9fe6c5a7a987ef06e5424792c817b77baf8914b8031f97674de4594`
...
Test date/time: 2026-09-18, ~20:50–00:40 local (UTC+2). Network: residential, Cloudflare-fronted hosts.
...
Also exercised: 1966 catalogue title in search, 2026 releases, multi-season shows, non-Latin titles, empty search, invalid ids, TV/Movies grid paging, Sub/Dub stream-label routing.
...
- **Site assertions: 44/44 passed** (`module-tests/site_checks.cjs`):
...
- **Fix shipped in beta.2:** community test identity `SP-VID-9004-STREAMINGUNITY-HERMES-TEST` / number `9004`
  ...
  (SP-VID-065).
- **Repository state (as of beta.5):** bundle `Testing-7` advertises beta.5; beta.1–beta.4 ZIPs remain on
...
| Repository index + bundle rebuild | 1 | 1 | 0 | `repository.json` rebuilt (bundle 4 at repair time; 5 for beta.3, 6 for beta.4, 7 for beta.5); module/bundle SHA-256 recomputed and checked |
...
| --- | --- | --- | --- |
| Site-side assertions (`site_checks.cjs`) | 44 | 44 | 0 |
...
- **Verified (local, this machine):** catalogue, search, details, season/episode lists, discovery rows+feeds, **home row order + counts after reservation**, **TV/Movies feed deep paging (pages 1–2 disjoint, strict type filter, guest cap), pagination without duplicates**, **Sub/Dub routing (labels + fetched master's DEFAULT flip byte-checked)**, stream resolution for 6 samples, crypto/segment structure, subtitle files + cue timing, ZIP integrity (bytes, hash, packaged smoke test), repository index/bundle rebuild + hashes, **in-app repository install of the repaired identity (operator side)**.
...
**Version:** 1.0.0-beta.5 · **ZIP:** `Testing-Modules-1234/modules/StreamingUnityHermes-1.0.0-beta.5.zip` · **SHA-256:** `a084a623e9fe6c5a7a987ef06e5424792c817b77baf8914b8031f97674de4594`
**Counts:** 87 attempted / 85 passed / 2 blocked-by-kit-guard (see table) · failure reasons: none from the module; the 2 blocked probes are the kit's deliberate AES-128 refusal
